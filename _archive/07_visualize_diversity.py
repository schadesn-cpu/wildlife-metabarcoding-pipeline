...
def compute_permanova(
    ...
    def f_stat(dist_mat, grp):
        """
Compute the PERMANOVA pseudo-F statistic from a distance matrix.

        Uses the Anderson (2001) formulation: between-group SS / within-group SS,
        each divided by their respective degrees of freedom. grp must be an
        integer array of group membership indices.
        """
        n = len(grp)
...
def compute_permdisp(
    ...
    def group_dispersions(dist_mat, grp_labels):
        """
Return a flat list of within-group dispersions for all samples.

        For each group, dispersion is approximated as each sample's mean
        distance to all other members of the same group (centroid proxy).
    ...
    def f_from_dispersions(disp, grp_labels):
        """
Compute the PERMDISP F-statistic from a list of per-sample dispersions.

        Between-group variance is the weighted sum of squared deviations of
        each group mean from the overall mean. Within-group variance uses ddof
...
def load_stats_tsv(tsv_path: Path, group_column: Optional[str] = None) -> Dict[str, Dict]:
    """
    Load a beta_stats_summary_*.tsv (produced by run_qiime_marker_pipeline.py).
    Returns {metric_label: {permanova: {F, p}, permdisp: {F, p}}}.

    If group_column is provided, only rows whose 'group_column' field matches
    (case-insensitive) are included. This prevents stats from a different grouping
    variable (e.g. Season) overwriting stats for the current run (e.g. Group).
    """
    stats: Dict[str, Dict] = {}
    METRIC_NORM = {
    ...
    with tsv_path.open(encoding="utf-8") as f:
        ...
        for row in reader:
            # Filter by group_column if TSV has that column and caller supplied one
            if group_column:
                row_gc = row.get("group_column", row.get("grouping_column", "")).strip()
                if row_gc and row_gc.lower() != group_column.lower():
                    continue
...
def plot_alpha(
    ...
    """
Generate a multi-panel alpha diversity strip/box plot and save as PNG + SVG.

    Each panel corresponds to one alpha diversity QZA (e.g. Faith's PD, Shannon,
    observed features, evenness). Points are jittered over a box, coloured by
    group. Statistical annotations are added automatically: Mann-Whitney U for
    two groups, Kruskal-Wallis for three or more.

    Args:
        alpha_qzas:   List of QIIME2 alpha vector QZA paths to plot.
        group_series: pd.Series mapping sample-id to group label.
        group_order:  Display order of group labels (left to right).
        palette:      Colour palette dict with keys 'colors' and 'markers'.
        marker:       Marker name used in the figure title and output filename.
        outpath:      Base output path; '_alpha.png' and '_alpha.svg' are appended.
    """
    import matplotlib
    matplotlib.use("Agg")
...
def plot_beta(
    ...
    """
Generate a multi-panel beta diversity PCoA plot and save as PNG + SVG.

    Each panel shows PC1 vs PC2 for one beta metric, with samples coloured by
    group. PERMANOVA and PERMDISP statistics are overlaid in a text box on each
    panel; they are loaded from stats_tsv if provided, otherwise computed on the
    fly from beta_dm_qzas using a pure-Python permutation implementation.

    Args:
        beta_pcoa_qzas: List of QIIME2 PCoA results QZA paths.
        beta_dm_qzas:   List of distance matrix QZA paths (for on-the-fly stats).
        group_series:   pd.Series mapping sample-id to group label.
        group_order:    Display order of group labels.
        palette:        Colour palette dict with keys 'colors' and 'markers'.
        marker:         Marker name for figure title and output filename.
        outpath:        Base output path; '_beta.png' and '_beta.svg' appended.
        stats_tsv:      Optional pre-computed stats TSV from parse_beta_stats.py.
        n_perm:         Permutations for on-the-fly PERMANOVA/PERMDISP (default 999).
        group_column:   Metadata column name used to filter rows in stats_tsv.
    """
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    ...
    if stats_tsv is not None and stats_tsv.exists():
        precomp_stats = load_stats_tsv(stats_tsv, group_column=group_column)
    ...
    for idx, (pcoa_qza, ax) in enumerate(zip(beta_pcoa_qzas, axes_flat)):
        ...
        def fmt(val):
            """Format a float to 3 decimal places, or 'n/a' if NaN."""
...
def parse_args() -> argparse.Namespace:
    """Build and parse the argument parser for 07_visualize_diversity.py."""
...
def main() -> int:
    """
Entry point for 07_visualize_diversity.py.

    Loads metadata, resolves sample IDs between artifact and metadata formats,
    then calls plot_alpha and/or plot_beta depending on which QZAs are provided.
    Saves PNG + SVG outputs to --outdir. Returns 0 on success, 1 on error.
    """
    args = parse_args()
    ...
    suffix += f"_{args.group_column}"         # always include grouping variable in filename
