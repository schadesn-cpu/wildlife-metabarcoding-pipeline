...
def read_permanova_qzv(qzv_path: Path) -> Optional[Dict]:
    """
    Parse a QIIME2 beta-group-significance QZV (PERMANOVA).
    Returns dict with keys: pseudo_f, p_value, or None if parsing fails.

    QIIME2 renders results as an HTML table with <th>/<td> pairs.
    The relevant keys (after lowercasing and underscore-joining) are:
      - test_statistic  → pseudo-F
      - p-value         → p-value
      - sample_size     → n (do NOT use as F-stat)
    """
    import re
    try:
...
def cmd_pcoa(args: argparse.Namespace) -> None:
    ...
    for artifact_path in artifacts:
        ...
        if stats_dir:
            ...
            # Build snake_case version of metric name for filename matching
            _pretty_to_snake = {
            ...
            snake_metric = _pretty_to_snake.get(metric_name, metric_name.lower().replace(" ", "_").replace("-", "_"))
            # Try both snake_case (actual filenames) and pretty name (fallback)
            candidates = [
    ...
    if args.panel and len(all_data) > 1:
        ...
        fig.tight_layout()
        stem = args.output_stem if args.output_stem else f"pcoa_panel_{args.color_by}"
        ...
        for coords, prop_expl, metric_name, color_col, stem, permanova_result in all_data:
            ...
            out_stem = args.output_stem if args.output_stem else f"pcoa_{stem}_{args.color_by}"
...
def plot_alpha(
    ...
    if group_order:
        _mask = meta_plot.isin(group_order)
        meta_plot = meta_plot[_mask]
        series_plot = series_plot[_mask]
    color_dict, categories = _get_color_map(meta_plot, palette)
...
def cmd_alpha(args: argparse.Namespace) -> None:
    ...
    if args.panel and len(all_data) > 1:
        ...
        last_ax = axes_flat[len(all_data) - 1]
        _, group_col_last, _ = all_data[-1][1], all_data[-1][2], all_data[-1][3]
        _meta_last = all_data[-1][2]
        if args.group_order:
            _meta_last = _meta_last[_meta_last.isin(args.group_order)]
        color_dict_last, group_labels_last = _get_color_map(_meta_last, palette)
        if args.group_order:
            group_labels_last = [c for c in args.group_order if c in group_labels_last]
        handles = [
        ...
        fig.tight_layout()
        out_stem = args.output_stem if args.output_stem else f"alpha_panel_{args.group_by}"
        _save_figure(fig, out_dir, out_stem)
        plt.close(fig)
        ...
        for series, metric_name, group_col, stem in all_data:
            fig = plot_alpha(
                series, group_col, metric_name, args.group_by, group_order=args.group_order,
                show_stats=not args.no_stats,
                palette=palette,
                show_title=not args.no_title,
            )
            out_stem = args.output_stem if args.output_stem else f"alpha_{stem}_{args.group_by}"
            _save_figure(fig, out_dir, out_stem)
            plt.close(fig)
...
Build and return the top-level argument parser for 06_plot_diversity.py.
    ...
    Two subcommands are available:
      pcoa  — 2D PCoA scatter plots from beta diversity QZAs, with optional
              95% confidence ellipses and a --panel mode for multi-metric figures.
      alpha — Strip/box plots from alpha diversity vector QZAs, with pairwise
              significance annotations (Mann-Whitney U or Kruskal-Wallis).
    """
    ...
    pc.add_argument(
        ...
        help="Base filename stem for output files (no extension). "
             "Default: pcoa_panel_{color_by}. "
             "Example: --output-stem 16S_r8000_DvT_pcoa_purple",
    )
    pc.add_argument(
        "--no-title", action="store_true",
        help="Suppress all panel and subplot titles (use for journal submission figures)",
    )
    ...
    al.add_argument(
        "--output-stem", default=None, metavar="STEM",
        help="Base filename stem for output files (no extension). "
             "Default: alpha_panel_{group_by}. "
             "Example: --output-stem 16S_r8000_DvT_alpha_purple",
    )
    al.add_argument(
        "--no-title", action="store_true",
        help="Suppress all panel and subplot titles (use for journal submission figures)",
    )
    al.add_argument('--group-order', nargs='+', default=None,
                          help='Explicit order of groups for x-axis (e.g. Diseased Trauma)')
...
Entry point for 06_plot_diversity.py.
    ...
    Parses the subcommand (pcoa or alpha), dispatches to the appropriate
    plotting function, and writes PNG + SVG outputs to --output-dir.
    Returns 0 on success, 1 on any unhandled error.
    """
    parser = build_parser()
